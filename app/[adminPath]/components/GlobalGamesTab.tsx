...
type Form = Omit<GlobalGame, 'id' | 'created_at'>
const EMPTY: Form = {
...
export function GlobalGamesTab({ adminSecret }: { adminSecret: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const d = await api('global-games', 'GET', undefined, adminSecret)
      setGames(d.globalGames || [])
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      if (editing) await api('global-games', 'PUT', { id: editing, ...form }, adminSecret)
      else await api('global-games', 'POST', form, adminSecret)
  ...
  const del = async (id: string, title: string) => {
    if (!confirm(`Delete game "${title}"? All model assignments will be removed.`)) return
    try { await api('global-games', 'DELETE', { id }, adminSecret); load(); show('Deleted') }
    catch (e: any) { show(e.message, 'error') }
  }

  const edit = (g: GlobalGame) => {
    setForm({
      title: g.title, slug: g.slug, description: g.description,
      page_description: g.page_description, logo_url: g.logo_url,
      seo_title: g.seo_title, seo_description: g.seo_description,
      og_image_url: g.og_image_url, sort_order: g.sort_order, is_published: g.is_published,
    })
    setEditing(g.id)
  }
  ...
  return (
    <div>
      ...
      <p style={s.secTitle}>{editing ? 'Edit Game' : 'Add Game'}</p>
      <p style={{ fontSize: 11, color: MUTED, marginBottom: 12 }}>
        Games are global reusable entities assigned to multiple models. Each gets its own page at /game/[slug].
      </p>
      <div style={s.card}>
        <div style={s.g2}>
          <Field label="Title">
          ...
          <Field label="Slug">
            <input style={s.input} value={form.slug || ''} onChange={e => set('slug', e.target.value)} placeholder="gran-turismo-7" />
          </Field>
        ...
        <div style={{ marginTop: 10 }}>
          <Field label="Page Description (shown on /game/[slug] page)">
            <textarea style={s.textarea} value={form.page_description || ''} onChange={e => set('page_description', e.target.value || null)} placeholder="Full editorial description for the game page…" />
          </Field>
        </div>
      ...
      <p style={{ ...s.secTitle, marginTop: 28 }}>All Games ({games.length})</p>
      {games.length === 0 && <EmptyState message="No games yet. Add your first game above." />}
      {[...games].sort((a, b) => a.sort_order - b.sort_order).map(g => (
        <div key={g.id} style={s.listItem}>
          <span style={{ flex: 1, fontWeight: 600 }}>{g.title}</span>
          <span style={{ color: MUTED, fontSize: 11 }}>{g.slug}</span>
          <span style={s.badge(g.is_published)}>{g.is_published ? 'Published' : 'Draft'}</span>
          <button style={s.btn()} onClick={() => edit(g)}>Edit</button>
          <button style={s.btn('danger')} onClick={() => del(g.id, g.title)}>Delete</button>
        </div>
      ))}
    </div>
  )

...
export function HomepageTab({ adminSecret }: { adminSecret: string }) {
  ...
  // Reassign clean sequential sort orders (fix gaps/duplicates)
  const normalizeOrder = async () => {
    if (!confirm('Normalize order? This will assign clean sequential numbers (10, 20, 30…) to all homepage photos.')) return
      ...
      <div style={{ ...s.row, marginBottom: 20, justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ color: MUTED, fontSize: 11, marginTop: 4 }}>
            These photos appear on the homepage in this order. Each has its own title shown on hover.
          </p>
        </div>
        <div style={s.row}>
          {photos.length > 1 && (
            <button style={s.btn()} onClick={normalizeOrder}>Fix order</button>
          )}
